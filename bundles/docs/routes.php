...
{
	return View::make('docs::page')->with('content', document('dmw'));
});
...
Route::get('(:bundle)/(:any)/(:any?)', function($section, $page = null)
{
	...
	// If no page was specified, but a "dmw" page exists for the section,
	// we'll set the file to the dmw page so that the proper page is
	// displayed back out to the client for the requested doc page.
	if (is_null($page) and document_exists($file.'/dmw'))
	{
		$file .= '/dmw';
	}
