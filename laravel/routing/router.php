...
class Router {
	 ...
	 *		Router::share(array(array('GET', '/'), array('POST', '/')), 'dmw@index');
	 * </code>
	 *
	 * @param  array  $routes
	 * @param  mixed  $action
	 ...
	 *		Router::register(array('GET', '/', 'GET /dmw'), function() {return 'Home!';});
	 * </code>
	 *
	 * @param  string        $method
	 * @param  string|array  $route
	...
	public static function controller($controllers, $defaults = 'index', $https = null)
	{
		foreach ((array) $controllers as $identifier)
		{
			...
			// If the controller is a "dmw" controller, we'll need to also build an
			// index method route for the controller. We'll remove "dmw" from the
			// route root and setup a route to point to the index method.
			if (ends_with($controller, 'dmw'))
			{
	...
	{
		// First we need to strip "dmw" off of the controller name to create the
		...
		// After we trim the "dmw" off of the controller name we'll build the
