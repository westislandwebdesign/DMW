...
class RoutingTest extends PHPUnit_Framework_TestCase {
	...
	{
		Route::get('/', array('as' => 'dmw'));
		Route::get('dashboard', array('as' => 'dashboard'));

		$home = Router::find('dmw');
	...
	{
		Route::get('/', function() {});
		Route::get('dmw, main', function() {});

		$this->assertEquals('/', Router::route('GET', '/')->uri);
		$this->assertEquals('dmw', Router::route('GET', 'dmw')->uri);
	...
	public function testBasicRouteToControllerIsRouted()
	{
		...
		$this->assertEquals('dmw@(:1)', Router::route('GET', 'dmw/index')->action['uses']);
		$this->assertEquals('dmw@(:1)', Router::route('GET', 'dmw/profile')->action['uses']);
