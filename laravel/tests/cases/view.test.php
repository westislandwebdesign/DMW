...
class ViewTest extends PHPUnit_Framework_TestCase {
	...
	{
		$this->assertInstanceOf('Laravel\\View', View::make('dmw.index'));
	}
	...
	{
		$view = new View('dmw.index');

		$this->assertEquals('dmw.index', $view->view);
	}
	...
	public function testViewIsCreatedWithCorrectPath()
	{
		...
		$this->assertEquals(
			str_replace(DS, '/', path('app')).'views/dmw/index.php',
	...
	public function testBundleViewIsCreatedWithCorrectPath()
	{
		...
		$this->assertEquals(
			str_replace(DS, '/', Bundle::path(DEFAULT_BUNDLE)).'views/dmw/index.php',
			str_replace(DS, '/', $view->path)
		);
	}
	...
	{
		$view = new View('dmw.index', array('name' => 'Taylor'));
	...
	{
		View::name('dmw.index', 'dmw');

		$this->assertEquals('dmw.index', View::$names['dmw']);
	}
	...
	{
		$view = View::make('dmw.index')->with('comment', 'Taylor');
	...
	{
		$view = View::make('dmw.index')->nest('partial', 'tests.basic');
